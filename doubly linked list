...
public:
	...
	{
		Node<T>* newNode = new Node<T>(nullptr,data, head);
	...
	{
		Node<T>* newNode = new Node<T>(tail,data, nullptr);
	...
	void insertAfter(const T& data, Node<T>* prev_node)
	{
		...
		{
			Node<T>* newNode = new Node<T>(prev_node,data, prev_node->next);
